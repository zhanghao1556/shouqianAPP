...
# 2026-07-20 AJ350 端子独立高亮分区

- 用户要求接口接线图按端子实际通道分区高亮：AJ350 的 SPK、LINE IN、LINE OUT 分别按面板内的 2×2 独立区域高亮，其他同类多路端子也不得因同属一个物理端子块而整块联亮；单独 3Pin 端子直接高亮完整端子外框。
- 本轮只调整端子悬停 / 选中高亮裁切范围，不修改接口能力、端子映射、连接关系、设备选型、数量或点位规则。
- 托管执行器仍把显式 PowerShell 7 路径解析为无权限的 WindowsApps `pwsh` alias，并报 `CreateProcessAsUserW failed: 5`。按项目规则停止重试，本轮回退 `cmd.exe`；中文项目文件仍以 UTF-8 内容为准。
- 首轮类型检查短暂发现生产入口未给 `InterfaceWiringPreview` 传入必需的 `interfacePanelImages`，阻断类型检查与构建；复查时工作区在途音翼接线改动已补齐音翼 / 音曼品牌包装组件和入口切换，问题随该在途实现恢复。本轮未覆盖或回退这些并行改动，后续检查均以最新工作区为准。
- 接口专项测试的前 20 项业务模型回归通过，随后旧源码形状断言因仍要求所有背板 SVG 文件名直接存在于通用渲染器而失败。资产已按品牌迁到音翼 / 音曼包装层，这条断言已失效；本轮将资产扫描改为覆盖通用渲染器与两个品牌包装层，并新增 AJ350 四路 SPK 分区、单 3Pin RS232 完整高亮框和不再整组联亮的结构断言。
- 首次更新品牌图纸 aria-label 断言时，在测试模块的外层 `String.raw` 模板内误放反引号字符，导致测试脚本解析失败。该错误由本轮新增断言造成并直接阻塞验证，已立即改为不含反引号的单行源码模式，不涉及实现或业务规则。
- 修正解析后，测试模块暴露在途音翼用例与既有音曼用例重复声明顶层 `controlEdge`，导致 esbuild 报符号重复。该问题阻塞专项测试，已只把音翼变量重命名为 `yinyiControlEdge`，断言内容和连接规则保持不变。
- 后续深比较发现首版 `anchor` 辅助函数给所有旧锚点附加了 `focusBounds: undefined`，运行显示无差异但破坏既有对象形状合同。已让普通锚点恢复原三字段结构，仅通过 `withFocusBounds` 给明确标定的端口添加高亮框。
- 用户追加明确端子高亮使用绿色。本轮把端子图像高亮层的静态与脉冲光晕统一改为绿色 `#22c55e`；线缆材质颜色、接口分区和连接联动不变。

# 2026-07-20 音翼阵麦端子丝印与横向拨片校准

- 用户提供 LINE OUT / LINE IN 实物丝印局部图并明确：两组 6Pin 端子都只按孔位标注 `G / L / R / G / L / R`，不得在背板上显示 `1 / 2 / 3 / 4` 或 `G / + / -`。主麦、从麦两张清晰 SVG 已同步；SPK 端子继续逐孔显示 `1+ / 1-` 至 `4+ / 4-`。
- 图面丝印与逻辑通道名称分离：占用模型仍用组号区分四个可独立分配的 L / R 信号，但每组 L、R 的两根回路线共接相邻 G 孔，未把 L / R 误作平衡正负。
- 用户补充主从拨片为横向布局，左侧 `M`、右侧 `S`。主麦图点亮左侧 M，从麦图点亮右侧 S，并以结构属性和专项回归锁定方向与选中状态。
- 音翼接口接线图成为正式双品牌能力后，同步修正通用发布 README、静态发布必需标记和浏览器业务一致性校验；后续音翼发布必须校验“音翼接口接线图”和接口占用表，同时继续禁止客户界面显示 `DT1 / DT2 / DT2 Pro` 等具体型号。本轮未打包发布。
- 自动检查通过：`test:interface-wiring`、`test:point-system`、严格 TypeScript、生产构建和 `git diff --check`。生产主包约 `696.88 kB`，继续触发既有 `500 kB` 非阻塞体积提示；单文件交付边界下本轮不借接口校准拆包。
- 最终 `npm.cmd run test:interface-wiring`、严格 TypeScript / `npm.cmd run build` 和 `git diff --check` 通过。专项测试已锁定绿色高亮、AJ350 四路 SPK 的 2×2 分区、LINE IN / LINE OUT 独立分区、单 3Pin RS232 完整端子高亮框及旧锚点对象兼容性。
- 5180 页面身份、非空渲染和接口图挂载均通过，控制台无应用错误或警告；页面实际计算样式确认静态与脉冲光晕均为 `rgb(34, 197, 94)`，SPK1 裁切框为独立小矩形。内置浏览器的指针移动未持续触发 React `pointerenter` 状态，未把该工具限制误判为页面失败，最终视觉悬停仍交由用户在当前 5180 标签页直接查看。
- 生产构建继续提示单一主包约 `696.88 kB`，超过 Vite `500 kB` 建议阈值。这是非阻塞的既有包体积问题，本轮只记录，不借端子高亮修改拆包或发布结构。

# 2026-07-20 端子绿色高亮可见性修正

- 用户截图确认上一版端子主体仍显示白色。根因是只把 `drop-shadow` 改成绿色，而端子原图仍在脉冲峰值使用 `brightness(1.82)`，绿色底色被大幅提亮成白色；绿色阴影又被裁切区限制，不能代表主体颜色。
- 本轮在每个独立端子裁切区内增加绿色半透明蒙层与绿色边线，使用混合模式保留孔位、编号和原图细节；同时把原图亮度限制在 `1.02-1.08`，不再把端子底色冲白。端子分区、接口锚点、线缆颜色和连接规则均不修改。
- 最终 `test:interface-wiring`、严格 TypeScript / 生产构建和 `git diff --check` 通过。5180 实际 DOM 中 24 个高亮分区均包含绿色主体蒙层；SPK1 计算样式为填充 `rgba(34, 197, 94, 0.48)`、边线 `rgb(22, 163, 74)`、`mix-blend-mode: multiply`，原图滤镜为 `brightness(1.02) contrast(1.12) saturate(1.18)`，控制台无应用错误或警告。
- 生产构建继续只有既有主包体积提示，当前主包约 `697.07 kB`；本轮不顺手拆包。内置浏览器指针移动仍不能保持 React 悬停态，因此未取得激活态截图，但实际高亮元素、计算样式和专项交互合同均已验证。
- 后续改动落入共享工作区后重新执行 `test:interface-wiring`、严格 TypeScript、生产构建和 `git diff --check`，全部通过；主包约 `697.07 kB`，仍只有既有的 `500 kB` 非阻塞体积提示。

# 2026-07-20 音翼单路非平衡输出接平衡录播输入修正

- 用户在 5174 接线图明确最终口径：阵麦主麦 `LINE OUT` 的单路 L 或 R 接录播凤凰 `+ / - / G` 时，两端都要把红、白并在同一个信号端；不能让红白在源端同相、目标端却分别进入 `+ / -`，否则会被差分输入抵消。
- 最终接法为：主麦端红白并接所选 L 或 R，屏蔽接同组 G；录播平衡端红白并接 `+`，屏蔽接 G，`-` 悬空。只修改该转换的线芯映射与占用表接线说明，不改变通道分配、端子位置、设备数量或点位规则。
- 本轮曾短暂把源端红白并接、目标端分别接 `+ / -`，用户立即指出抵消风险；该错误改法在测试阶段即被撤销，未提交。
- 用户补充确认阵麦接录播 3.5mm 或凤凰 `L / R / G` 时也只送一个单声道：主麦端红白并接同一个 L 或 R、屏蔽接同组 G；录播端红接 L、白接 R、屏蔽接 G，三个端子全部接线，使左右声道获得同一份单声道信号。三种录播输入模式均只占用阵麦一个独立 LINE OUT 信号通道。
- 最终 `test:interface-wiring`、严格 TypeScript / 生产构建和 `git diff --check` 通过。回归分别锁定平衡端红白同接 `+` 且 `-` 无线、3.5 与 L/R/G 端红接 L / 白接 R / 屏蔽接 G，以及三种模式源端都只使用一个 `lineOut` 信号。主包约 `697.58 kB`，仍只有既有体积提示。
- 用户要求从麦拨片选中 S 时也用红色，便于与主麦红色 M 形成一致的选中语义。已把从麦 S 由蓝色改为红色，未选中 M 保持灰色；只修改背板表现，不改变主从逻辑或级联规则。

# 2026-07-20 音翼 AJ200 / AJ600 处理器背板映射

- 用户在 5174 指出音翼“智能音频处理主机”显示“接口图待补充”，并确认音翼处理器与音曼硬件相同：双麦档使用 AJ200、六麦档使用 AJ600，音翼没有 AJ350。
- 根因是音翼接口模型未给正式设备清单中的处理器名称映射具体端口档案，通用兜底错误落到 AJ350；同时音翼背板资源表未注册 AJ200 / AJ600 清晰图。
- 本轮只同步处理器物理档案和共用背板：音翼双麦→AJ200、六麦→AJ600、未知通用处理器→AJ200 安全兜底，并明确禁止音翼接口模型生成 AJ350。设备清单中的处理器选型、数量、音箱 / 阵麦规则和点位均不修改。

# 2026-07-20 音翼外部音源接阵麦 LINE IN 并芯修正

- 用户指出当前无线接收机平衡输出接阵麦 `LINE IN` 仍把负相芯接到了阵麦共地，并再次确认凤凰平衡、卡侬和 6.35 都必须采用同一单声道并芯原则。
- 最终口径：阵麦端红白并接同一个 L 或 R，屏蔽接同组 G；平衡凤凰端红白并接 `+`、屏蔽接 G、`-` 悬空；卡侬端红白并接 pin 2（+）、屏蔽接 pin 1（G）、pin 3（-）悬空；6.35 TS 端红白并接 TIP（信号）、屏蔽接 SLEEVE（G）。不得把红白分别接到平衡正负端。
- 本轮 PowerShell 7 路径被托管执行器改写为无权限的 WindowsApps alias 并报 `CreateProcessAsUserW failed: 5`，已按项目规则回退到 `cmd.exe`，中文与结构化文件继续用 Node UTF-8 读取；该问题属于执行器边界，不是项目文件损坏。
- 已统一正式连接映射与占用表说明：凤凰和卡侬只使用正端与地端，6.35 TS 新增 TIP / SLEEVE 物理端子语义；插头端仍由既有成品接头图表示，不展开线芯，阵麦凤凰端保留红白同孔和屏蔽接 G 的清晰分叉。
- 专项回归分别使用正式生成的 `wireless-receiver-line-dt`、`microphone-line-dt-1` 连接验证凤凰、6.35 和卡侬两端逐芯关系，并确认负相端未占用。`test:interface-wiring`、`tsc && vite build` 与 `git diff --check` 通过；主包约 `698.84 kB`，仍只有既有的 500 kB 体积提示。本轮按用户验收节奏未追加浏览器代验。

# 2026-07-20 音翼外部音源接阵麦端改为白线悬空

- 用户复核后确认不再在阵麦端并接红白：平衡凤凰或卡侬接头侧可保持红接正相、白接负相、屏蔽接地，阵麦端只把红线接所选 L 或 R、屏蔽接同组 G，白线绝缘悬空；6.35 TS 只使用 TIP 红线与 SLEEVE 屏蔽，白线两端不接。
- 原实现把红白都映射到阵麦同一个信号孔，虽然不会在当前映射中形成正负抵消，但隐藏接头内部接法后施工容错较低。正式图应只把有连续电气连接的红线与屏蔽线画到阵麦端，并在占用表明确白线的悬空位置。
- 已将三类输入的活动导线统一缩减为红色信号线与屏蔽线两根：平衡凤凰 / 卡侬说明保留源端白线接负相、阵麦端白线绝缘不接；6.35 TS 说明明确白线两端不接。专项回归确认阵麦端不存在白色导线或负相端点，`test:interface-wiring`、`tsc && vite build`、`git diff --check` 通过；主包约 `698.68 kB`，仍只有既有体积提示。

# 2026-07-20 原有音频系统接口接线图暂缓绘制

- 用户确认音翼、音曼共同规则：客户选择原有音频系统后，接口接线图暂时不绘制原有音频系统节点及相关连接，因为会形成两个一级设备且现场接法复杂。
- 接线画布必须直接标注“接入原有音频系统较为复杂，建议联系对应FAE处理。”；原有音频系统仍保留在采集参数、设备判断、拓扑和其他业务输出中，本轮只改变接口接线图及其占用表投影。
- 实现必须同时屏蔽新主机接原系统、原系统内部链路，以及电脑、录播、无线接收机等被规则改接到原系统的连线；不得让接口模型的录播 / 电脑兜底逻辑把这些设备错误接回新主机。
- 已在共享接口模型中按正式连接 ID 和利旧设备端点过滤上述连接，并在录播 / 电脑节点预建及兜底接线前检查其是否已被路由到原系统；原始 `outputs.connectionLines` 不变，因此拓扑及其他业务输出仍保留利旧关系。接口模型增加 `review` finding，接线 SVG 标题区使用纯 SVG 琥珀色提示条显示指定 FAE 文案，可进入 PNG / PDF 导出。
- 首次双品牌回归把“讲台电脑”按标签一律视为利旧节点，误判了音曼仍通过新系统 USB 正常连接的共享电脑。已把合同收紧为只禁止利旧连接 ID、对应占用行及调音台 / 原音频处理器 / 反馈抑制器 / 原功放 / 原音箱等利旧专属节点；共享设备只有在没有新系统连接时才消失。
- 托管执行器把显式 PowerShell 7 路径改写为无权限 WindowsApps alias 并报 `CreateProcessAsUserW failed: 5`，本轮按项目规则仅回退一次到 `cmd.exe`，中文规则和日志均以 UTF-8 内容核对，未误判为文件损坏。
- `npm.cmd run test:interface-wiring`、`tsc && vite build`、`git diff --check` 均通过。生产主包约 `700.21 kB`，仍只有既有的 `500 kB` 体积提示，本轮不借接线规则修改拆包策略。

# 2026-07-20 接口高亮按凤凰端子与普通连接器分流

- 用户截图指出 RJ45 网口也被绿色 tint 覆盖，随后明确纠正：只有凤凰 / 接线端子使用绿色高亮，网口及其他普通连接器恢复原来的接口显现方式。
- 首次理解成“只保留端子高亮、取消其余悬停反馈”，曾在未测试前临时移除设备背板降亮、非当前线路淡化和当前线路光晕；用户及时纠正后已恢复这些原有反馈，没有把错误理解作为正式规则保留。
- 历史实现显示普通连接器原效果为接口裁剪区域提亮，并由白色光晕过渡到浅蓝色；后续绿色改动把同一效果误套到了所有接口。正式实现按 `terminalSelective` 分流：凤凰 / 接线端子继续渲染绿色整块 tint，RJ45、USB、3.5、6.35、卡侬等普通连接器不生成绿色层，恢复白蓝局部提亮。
- 本轮不修改端子裁剪范围、接线关系、线材颜色、线路路由或接口占用规则。
- 首次新增的端子动画负向正则没有限定 CSS 规则边界，跨过后续样式后误命中普通连接器的 `brightness(1.82)`，导致专项测试假失败；已改为正向锁定端子动画自身的 `1.02 -> 1.08` 两档亮度。
- 自动校验确认当前音翼画布包含 22 个端子高亮分区和 6 个普通连接器分区，22 个绿色 tint 全部位于端子分区，普通连接器内为 0；用户随后在 5174 页面明确验收“没问题”。内置浏览器无法稳定保持 React 指针悬停态的已知限制不再重复尝试，以结构合同和用户实际交互验收为最终视觉依据。
- `npm.cmd run test:interface-wiring`、生产构建和 `git diff --check` 通过。生产主包约 `700.36 kB`，继续只有既有的 `500 kB` 非阻塞体积提示。本轮显式 PowerShell 7 再次被托管执行器解析到无权限 WindowsApps alias，按项目规则停止重试并回退 `cmd.exe`。

# 2026-07-20 音翼阵麦到功放改为成对输入短跳

- 用户验收接口高亮后指出音翼阵麦到教学模拟功放的现图不应把一根音频线拆出多组线芯直接接两个通道；正式接法改为外部音频线先接 `LINE IN 1`，再在功放端子内用一根小短线从 `LINE IN 1` 跳到 `LINE IN 2`。
- 阵麦 `L / R` 可作为两路独立信号的产品能力保持不变；本次只改变扩声功放的分配方式。四通道功放按成对规则处理：第一根阵麦 LINE OUT 信号接 `LINE IN 1`、短跳 `1 -> 2`；需要第三、四通道时第二根信号接 `LINE IN 3`、短跳 `3 -> 4`。
- 外部音频线的真实电气映射继续保留在结构化数据和接口占用关系中：所选 L 或 R 信号接功放正端，共地 G 并接负端和地端；图面使用整根成品音频线，不展开红 / 白 / 屏蔽线芯。端子短跳仍保存 `+ / - / G` 一一对应关系，但图面只显示单根短弧线。
- 接线说明和拓扑侧 Line Out 数量口径已同步为“每根 Line Out 接奇数 LINE IN，再短跳到相邻偶数 LINE IN”；音箱选型、数量、分组、阵麦数量和点位规则均未修改。
- `npm.cmd run test:interface-wiring`、严格 TypeScript / 生产构建和 `git diff --check` 通过。当前合并工作区主包约 `703.83 kB`，仍只有既有的 `500 kB` 非阻塞体积提示。
- 校验期间工作区出现另一项未由本轮编辑的吸顶音箱背面图改动，涉及品牌包装层、端口目录、共享接线模型和未跟踪 SVG；本轮保留并兼容该改动，不覆盖、不回退，Git 存档只暂存功放短跳相关行。

# 2026-07-20 吸顶音箱背面接线线稿

- 用户要求新增吸顶音箱说明书式背面图，接线方式与壁挂音箱一致。本轮只修改接口图稿和接口面板身份，不修改音箱选型、数量、点位、角度或阵麦规则。
- 仓库现有 `topology-ceiling-speaker.png` 只提供吸顶音箱正面的圆形外缘和同心环证据，没有背面实拍。新图因此保留实物可确认的圆形比例，背壳采用简化说明书示意；删除了无背面证据的卡爪、螺钉等机械细节。双弹簧端子沿用已确认的壁挂结构，保持左 `-`、右 `+`，图内不画已接上的线。
- 新增纯矢量灰阶共享资源 `ceiling-speaker-rear.svg`，无 `<image>` 或嵌入位图；新增独立 `CEILING-SPEAKER` 接口 profile 和 `ceilingSpeaker` 资源键。音翼、音曼入口均注册该通用资源，吸顶节点按 `speakerSelection.productId` 映射，壁挂节点继续使用原 `PASSIVE-SPEAKER` 背板；共享文件名不携带任一品牌前缀。
- 接线层继续自动绘制红线 `+ -> +`、白线 `- -> -`，连接方法保持“保持正负极一致”。四个吸顶音箱分组在桌面实测均使用 112 x 112 背板，未混入壁挂背板。
- 方形背板暴露了紧凑音箱节点原先 `Math.max(120, nodeWidth - 20)` 可能让图片比 112px 卡片宽 8px 的问题；已在图片宽度计算中增加 `nodeWidth` 硬上限。520px 浏览器实测内容宽 505px、接线画布 441px，页面和画布均无横向溢出，所有背板位于所属卡片内。
- 浏览器实测流程为 5180 音曼页切换“吸顶音箱”后检查接口接线图：页面只出现 4 个吸顶音箱背板、0 个壁挂背板，极性图例“红线接 +；白线接 -”可见，Vite 错误层和控制台错误均为 0。桌面与 520px 窄屏截图均通过。
- 托管执行器再次把显式 PowerShell 7 重定向到无权限 WindowsApps alias，按项目规则回退 `cmd.exe`。首次沙箱内专项测试又因 esbuild 无权读取项目目录失败，随后按既有权限路径重跑通过；两者均属于执行环境限制，不是源码错误。
- `npm.cmd run test:interface-wiring`、`npm.cmd run test:point-system`、`npm.cmd run build` 和 `git diff --check` 通过。生产主包约 `703.61 kB`，仍只有既有的 `500 kB` 非阻塞体积提示，本轮不借图稿任务调整拆包策略。

# 2026-07-20 音曼手动双麦处理器全链路一致性

- 用户确认处理器口径：自动选择单只智能线阵麦克风时使用 AJ350；客户手动选择“双麦处理器”后，设备清单、拓扑图、接口接线图和接口占用表必须全链路统一使用 AJ200。手动六麦同理保持 AJ600；大圆盘阵麦仍受只能使用 AJ350 的既有硬规则约束。
- 当前 5180 草稿已保存手动 `twoMic`，正式设备清单和拓扑因此正确选择 AJ200；但接口接线构建器又独立执行候选判断，并把“单只线阵麦”无条件强制为 AJ350，覆盖了正式手动选择。这就是图中出现 AJ350 四组 SPK、AJ200 两组 MIC 消失，并产生错误“MIC 输入不足”提示的共同根因。
- AJ200 端口档案本身无误，已包含 2 组 MIC、2 组 SPK 和 HP IN / HP OUT。3.5 接线结构也已保存红 `L -> +`、白 `R -> +`、屏蔽 `G -> G` 三条凤凰端线芯；图上看到的一根绿色线是成品音频线胶套，3.5 插头侧按既有规则折叠线芯。该显示现象不是电气映射缺芯，后续只在 AJ200 正确选中后复核接口呈现。
- 本轮先按项目边界记录原因，再修正接口接线层的处理器选择来源并补自动 AJ350 / 手动 AJ200 回归；不修改音箱数量、音箱点位、阵麦数量或阵麦点位规则。
- 新增回归首次运行时，处理器选择、AJ200 档案和自动 AJ350 边界均已通过，但测试把正式模型会添加 `candidate-` 前缀的有线麦连接 ID 写成完全相等，导致断言找不到已存在的连接。该问题由本轮测试代码造成并阻塞验证，立即改为匹配稳定业务 ID 后缀，不涉及接线规则。

# 2026-07-20 接线图设备名称与拓扑图同步

- 用户要求接口接线图的设备名称与系统拓扑图一致。5174 实页只读对照确认，两张图当前确实使用两套命名来源：拓扑图显示“主麦 / 从麦 / 功放 / 有线麦”等工程简称，接线图则直接读取接口档案的“智能天花阵列麦克风（主麦）/ 教学模拟功放主机 / 利旧有线麦克风”等名称。
- 本轮只统一客户可见节点名和由节点名派生的接口占用表设备名：音翼单阵麦为“阵麦”，多阵麦为“主麦 / 从麦 n”；音曼阵麦、正式处理器名称、功放、有线麦、手持麦、笔记本、会议终端和一体机类名称跟随拓扑现有简称。音箱仍保留接线图必须使用的组号与只数后缀，用于辨认具体端口，但基础设备名继续与拓扑一致。
- 不修改接口档案 `customerName`、内部型号、设备清单、拓扑规则、设备数量、音箱分组、接口分配或接线关系；接口档案名称仍作为资料身份，接线模型在生成节点时做拓扑显示名归一化。

# 2026-07-20 接线端呈现与设备清单自动恢复入口

- 用户复核 5174 后指出三项问题：阵麦 `LINE OUT` 到功放的端子线芯消失；RJ45 成品连接不应继续显示线芯，且现有“白橙 / 白绿”以纯黄色、纯浅绿色代替条纹芯并不准确；设备清单缺少统一恢复自动推荐的入口。
- `LINE OUT -> 功放` 的结构化三芯映射仍完整，消失只发生在图面。根因是上一轮实现“功放端子内短跳不展开线芯”时，把 `conductorDisplay: collapsed` 错误加到了外部现场音频线，而不是只限制 `LINE IN 1 -> 2`、`3 -> 4` 的设备内短跳。正式修正只恢复外部音频线红 / 白 / 屏蔽分叉，内部短跳继续显示一根短线。
- RJ45 底层按 T568B 保存 1-8 直通映射；检索资料再次确认顺序为白橙、橙、白绿、蓝、白蓝、绿、白棕、棕，“白橙 / 白绿”等为白底带对应色条，不能用单一黄色或浅绿色冒充。客户图改为两端明确显示 RJ45 水晶头并折叠全部芯线；结构化针脚映射继续保留供接口校核，不在图面显示错误的扁平线芯色。
- “恢复自动推荐”仅归零设备清单里的手动数量覆盖，并把处理器手动档位恢复为 `auto`；麦克风方案和音箱方案已有各自独立的恢复按钮，本入口不擅自改动它们，也不改变音箱 / 阵麦选型、数量和点位底层规则。
- 本轮显式 PowerShell 7 再次被托管执行器重定向到无权限 WindowsApps alias 并报 `CreateProcessAsUserW failed: 5`，已按项目约定停止重试并改用 `cmd.exe`；中文规则、日志及结构化内容继续按 UTF-8 核对，未误判为文件损坏。

# 2026-07-20 音翼线阵麦接口接线链缺失

- 用户在 5174 发现音翼线阵麦方案缺少线阵麦、音箱和中控 232 等多组节点，并确认品牌边界：音翼线阵麦方案除不允许 AJ350 外，应与音曼共用 AJ200 / AJ600 的处理器、音箱、外接设备和 232 接线能力。
- 实页只读检查确认当前图中只有双麦处理器、功放、讲台电脑、有线麦和中控节点，边列表仅剩 4 条零散正式连接，没有线阵麦节点、音箱边或 232 边。该结果不是布局裁切或图层隐藏。
- 根因是共享构建器按品牌直接分流：只要 `brandId === yinyi` 就固定调用 DT2 Pro 自带处理器阵麦准备函数；线阵麦方案的 `DT2-Pro` 数量为 0，该函数立即返回，却没有回落到共享处理器准备函数。后续音箱和中控逻辑又继续按音翼品牌只查找 `dt-main`，因此分别跳过音箱路由和 232 路由。
- 修正边界：仅当音翼方案确实存在 DT 自带处理器阵麦时使用 `dt-main` 专属链；音翼线阵麦 / 吊麦改用共享 AJ200 / AJ600 链，处理器选择层硬排除 AJ350，音箱和中控根据实际存在的一级设备选择 DT 或处理器。音曼逻辑、DT 专属级联、音箱 / 阵麦选型数量与点位规则均不修改。
- 全链修正后页面仍把智能线阵麦克风显示为“接口图待补充”。第二层根因是音翼品牌入口虽然已经注册共享 AJ200 / AJ600、功放和音箱面板，却漏注册同一产品使用的 `lineArray` 与 `lineArrayConverter` 面板资源；已补齐这两个共享资源，继续明确不导入 AJ350。
- 新增音翼独立线阵整链回归：手动双麦场景必须同时存在智能线阵麦克风、AJ200、处理器直连音箱、功放扩展音箱和处理器到中控的 232 连接，且不得出现 AJ350；品牌入口回归同时锁定线阵麦及拓展器面板已注册。
- 验证通过：`test:interface-wiring`、`test:point-system`、两次生产构建和 `git diff --check`。5174 实页切换到线阵方案后显示 12 组设备、15 条线路，包含完整线阵麦背面板、双麦处理器、功放、12 只壁挂音箱和中控 232；无接口占位图、无 AJ350、控制台无错误或警告。验证结束后已恢复用户原“智能天花阵列麦克风”选择。
- 本轮托管执行器再次把默认 PowerShell 入口解析到无权限 WindowsApps `pwsh` alias，按既定兼容边界改用可用的系统 PowerShell 启动器，并由 Node UTF-8 读取中文文件；没有把该环境失败误判为文件损坏。生产构建仍只有既有的主 JavaScript 包超过 `500 kB` 提示，本轮只记录，不借接线修复顺手拆包。

# 2026-07-20 音翼功放邻近音箱链路布局待确认

- 用户在音翼三只阵麦、12 只壁挂音箱场景指出功放离主麦和所属音箱过远，导致主麦到功放、功放到音箱的线路过长并穿过主麦区域，图面难以判断。
- 根因位于接线图紧凑布局：全部音箱先组成独立 `speaker-band`，功放则继续作为普通二级设备参与剩余设备行打包；候选布局评分优先比较总高度，再比较交叉和总线长，没有把功放视为“上游一级设备与所属音箱之间的中继设备”，因此当前最紧凑候选把功放放到左下空位。
- 拟新增通用中继布局规则：存在功放及其所属音箱时，音箱仍按编号连续成带且一级设备仍居中；功放优先放在上游一级设备与其负责的音箱组之间，并在水平方向靠近所属音箱组。主麦直驱音箱线和功放音箱线分成独立曲线扇区，不改变设备数量、端口分配、音箱编号、接线关系或线材颜色。
- 当前案例拟调整预览为 `outputs/rule-previews/260720-yinyi-amplifier-near-speaker-chain-preview.svg`，已明确标记“拟调整预览 / 尚未写入正式规则”。预览中功放位于第 5-12 只音箱正下方，主麦直驱第 1-4 只走左侧，功放四路音箱线就近向上；等待用户确认后再修改正式布局算法。
- 独立 SVG 在 in-app Browser 中正确显示全部六组音箱、主麦、功放及拟调整标记；浏览器工具仍记录既有的独立 SVG `animation` 检查异常，图片本身正常，本轮不为该工具问题修改应用代码。

# 2026-07-20 音翼功放邻近所属音箱带正式实现

- 用户明确确认 `260720-yinyi-amplifier-near-speaker-chain-preview.svg` 的布局方案，正式接线图新增通用中继布局：一级设备保持画布中心，全部音箱组继续按编号连续排列；拥有下级音箱的功放从普通设备行中提取，在一级设备与音箱带之间单独排序，并横向靠近其负责的音箱范围。
- 首版正式实现的中继纵向顺序回归已经通过，但 `993px` 通用布局回归发现“功放与线阵麦重叠”。根因是功放虽先建立独立中继行，后续紧凑打包仍允许普通设备加入该行；功放完成行内排布后又直接移到所属音箱组中心，没有把同排线阵麦作为横向障碍物。
- 已把中继横向对齐改为碰撞感知：先计算所属音箱组中心，再在画布普通设备边距内枚举目标位置、原安全位置和每个同纵向设备两侧的合法位置，保持 `24px` 设备间距并选择离音箱中心最近者；没有目标侧空间时保留原行布局位置。纵向“一级设备 -> 功放 -> 所属音箱带”顺序和一级设备居中均保持不变。
- 本轮只修改接口接线图布局及对应回归，不改变设备数量、音箱数量与编号、阵麦数量、端口分配、接线关系或线材颜色。按用户验收节奏未追加浏览器代验，交由用户在当前 5174 页面做业务视觉确认。
- 自动验证通过：`npm.cmd run test:interface-wiring`、`npm.cmd run test:point-system`、`npm.cmd run build` 和 `git diff --check`。专项回归覆盖 `520 / 993 / 1120px` 节点不重叠、一级设备几何居中、功放位于一级设备与所属音箱带之间，以及功放中心朝所属音箱范围对齐。生产主包约 `707.89 kB`，仍只有既有的 `500 kB` 非阻塞体积提示。

# 2026-07-20 音频线胶套与端子落点异常待修复

- 用户在 5180 音曼接口接线图中发现利旧无线接收机到高性能处理器的 6.35 音频线退化为细线，并在处理器端视觉落到 `-` 端子，要求检查全部音频线，避免同类错误继续出现。
- 该问题同时可能影响客户对线材类型和端子极性的判断，属于当前接线图功能的阻塞性可见 bug。已暂停 Git 暂存，下一步分别审计音频边的结构化导线映射、成品接头折叠条件、胶套主干渲染和端子锚点选择；在查清共同根因前不修改设备、音箱或阵麦业务规则。
- 共同根因已确认：6.35 TS 端只有 `signal / ground` 两种逻辑角色，处理器 LINE IN 为 `positive / negative / ground`；旧通用映射先按相同 ID / 角色匹配，只匹配到 `ground` 后便错误提前返回，导致信号芯丢失、整条线只剩一根 `2.2px` 地线。截图中看似落到负极的细线实际来自这条不完整映射，仍足以形成错误施工判断。
- 已建立通用模拟音频接口转换矩阵，覆盖平衡、L/R/G 立体声和单声道三类端口的双向组合；本例固定为 6.35 `TIP -> LINE IN +`、`SLEEVE -> G`、`LINE IN -` 悬空。通用映射不再接受只匹配到部分源端子的模拟音频边，已知接口的每根活动芯必须落在真实端子上。
- 渲染层增加独立兜底：所有现场模拟音频线胶套固定使用 `6px`，不再由当前活动芯数决定粗细；设备端子内部已确认的 `LINE IN 1 -> 2` 小短跳继续保持 `3.2px`，没有被误放大。端子附近的红线 / 白线 / 屏蔽线仍按 `2.2px` 展开，便于辨认实际孔位。
- 专项回归新增全部代表模型的模拟音频线完整性审计：排除 USB Audio 与设备内部短跳后，每条音频线 / 话筒线至少两根活动芯；有端子资料时逐芯校验端点存在；单声道接平衡输入必须存在“信号到正极、地到地”，并禁止占用负极。利旧 6.35 场景另锁定 `tip-positive / sleeve-ground` 和占用表悬空说明。
- 5180 实页核验通过：`candidate-processor-wireless-receiver-audio` 主胶套颜色为音频线绿色 `#0f766e`、线宽 `6`，端子路径仅为红色 `tip -> positive` 与屏蔽色 `sleeve -> ground`，无 `negative` 路径；当前图六条现场模拟音频线线宽全部为 `6`，控制台错误 / 警告为 0。
- 最终 `npm.cmd run test:interface-wiring`、`npm.cmd run test:point-system`、`npm.cmd run build` 和 `git diff --check` 全部通过。生产主包约 `709.68 kB`，继续只有既有的 `500 kB` 非阻塞体积提示；本轮未修改设备、音箱、阵麦、端口分配或线路拓扑规则。

# 2026-07-20 双品牌 2.0 正式版与高清报告发版审计

- 用户确认当天工作完成，要求先校准音翼、音曼导出报告，重点保证接口接线图放大后清晰；确认无误后按完整流程发布双品牌 `2.0`，并删除新版本中的“内部测试版”客户可见字样。
- 发版前审计发现：接口接线 SVG 统一按 `2x` 栅格化，但整个 PDF 页面最终固定为 `1240 x 1754px`，约为 A4 150 DPI；即使源图较大，接线页仍会在页面合成时降到该分辨率。拟只把接口接线图源图提高到 `4x`、接口接线报告页提高到 `2480 x 3508px`（A4 约 300 DPI）并使用更高 JPEG 品质，其他页面保持原分辨率，避免整份报告体积无差别翻倍。
- 发版链存在分散硬编码：`package.json`、单文件构建、通用包构建、静态验证、业务一致性验证、移动端验证、PDF 元数据与报告文件名仍使用 `1.1`、`internal-test` 或“内部测试版 / 内部测试报告”。本轮统一升级为 `2.0` 正式版，并在最终 HTML 增加结构化发布版本标记；旧发布归档保留，不执行破坏性删除。
- `package-lock.json` 顶层版本仍停留在 `0.1.0`，与此前 `package.json` 的 `1.1.0` 已不一致。该版本元数据问题直接影响本次 `2.0` 发版，已按用户明确要求当场修正并记录，不等待后续清理。
- 托管执行器再次把显式 PowerShell 7 路径解析到无权限 WindowsApps alias 并报 `CreateProcessAsUserW failed: 5`；按项目规则停止重试，本轮简单命令使用 `cmd.exe`，中文文件继续按 UTF-8 核对。该现象属于执行器启动限制，不代表源码或 PowerShell 安装损坏。
- 已实现接口接线图专属高清链：接口 SVG 按 `4x` 取样，接口接线报告页单独使用 `2480 x 3508px` A4 画布和 `0.98` JPEG 品质；点位图、拓扑图、档案页和接口占用表继续使用 `1240 x 1754px`，避免整份报告无差别增大。
- 报告载荷版本升级为 `2.0`，导出文件名统一为“项目名-售前方案报告.pdf”，报告页脚删除“内部”限定并保留售前方案参考边界；工程输出模型的报告副标题改为“售前工程方案报告”。
- 使用全新 Playwright 浏览器上下文分别打开 5174 和 5180，录入同一组 `18m x 9m x 3m` 普通教室本地扩声参数并实际下载 PDF。最终文件为 `output/pdf/音翼AI售前工具-2.0-导出报告校准.pdf` 与 `output/pdf/音曼AI售前工具-2.0-导出报告校准.pdf`，大小约 `1.22 MB / 1.25 MB`。
- Poppler 与 pypdf 检查确认两份报告均为 5 页 A4、品牌 Producer 正确、可回导载荷版本为 `2.0`；第 4 页接口接线图内嵌图像为 `2480 x 3508px`，其余页为 `1240 x 1754px`。两品牌全部页面以 150 DPI 渲染逐页检查，并对接线页以 300 DPI 再渲染放大检查；未发现裁切、重叠、黑块、乱码、页码错误或品牌串用，端口名、极性、线材胶套和图例放大后可辨认。
- in-app Browser 对 Blob 下载没有触发可用的 download 事件，等待 120 秒后超时，但页面控制台和 JS 对话框均无错误；改用项目已安装 Playwright 的全新本地上下文后，两份报告在约 8 秒内正常完成。该问题归类为浏览器工具下载通道限制，不修改业务代码。
- 首次在沙箱内执行生产构建时，esbuild 因无权读取项目上级目录而无法解析 `vite.config.ts`；按项目既有发布流程在授权环境重跑后通过。生产主包约 `709.91 kB`，继续只有既有的 `500 kB` 体积提示。
- 首次重跑接口专项时，所有业务模型断言均已通过，但报告源码形状断言仍要求 `renderImagePage("接口接线图", interfaceWiringImage)` 的旧两参数调用，因此把新增高清选项误报为失败。该测试直接阻塞当前发版，已立即更新为锁定 `4x` SVG、`2x` 页面、`0.98` 品质和可配置栅格比例；不涉及设备、点位或接线业务规则。
- 收工清理发现 `.gitignore` 只放行带“内部测试版”的旧发布目录；新的 `音翼/音曼AI售前工具-2.0-日期-序号` 最终目录会被 `outputs/*` 误忽略，导致 release checkpoint 只有 zip 而缺少解压目录。该问题直接违反发布包本地提交要求，已保留旧归档规则并单独放行双品牌 `2.0` 最终目录；中间单文件目录仍保持忽略。
- `git diff --check` 无空白错误，只提示 `package.json` 与 `package-lock.json` 的工作区 CRLF 会在 Git 写入时转换为 LF。该提示不影响 JSON 内容、npm 解析、类型检查或构建，本轮不为此重写其他文件或扩大格式化范围。
- 首次 `release:all` 已按新路径生成双品牌 `2.0-260720-1` 目录和 zip，但静态验证正确停止交付。失败来自两条旧验证假设：压缩 HTML 必须包含完整的“品牌名+接口接线图”连续字面量，以及音翼 HTML 原始源码不得包含 AJ / SA110 / AP150 内部型号。当前品牌前缀在运行时组合，且 AJ200 / AJ600 已是音翼正式接线链内部共享硬件；内部代码型号允许存在，禁止的是客户可见泄露。
- 已删除静态验证中脆弱的连续字面量要求，并把型号隐藏责任收紧到全新浏览器上下文的客户可见文本检查；音翼可见文本黑名单同步增加 AJ200 / AJ350 / AJ600 / SA110 / AP150。原始 HTML 继续校验标题、品牌标记、`2.0` 标记、关键 CSS、接口接线 / 占用通用文案、跨品牌资产、内部测试版、乱码和预览残留。
- 用户要求客户端所有“张灏”统一改为“FAE”。已定位客户页面底部参考提示和 PDF 报告页脚两处正式来源并统一替换；历史日志中的事实记录不做追溯改写。
- 双品牌最终 HTML 静态验证和全新浏览器客户可见文本验证均新增“张灏”禁用检查，接口专项测试同步锁定页面与 PDF 只能显示“联系FAE”，防止后续回归。
- 本轮续接时托管执行器再次将显式 PowerShell 会话解析到无权限 WindowsApps alias；按既有项目规则只记录一次并继续使用 `cmd.exe` 与 UTF-8 工具，不将其误判为项目问题。
- 接口专项首次续跑时所有业务模型断言均通过，但仍有一条测试要求发布验证源码出现“品牌名+接口接线图”连续表达式，与已确认的运行时品牌拼接实现冲突。已把该测试同步为检查通用“接口接线图”标记，完整品牌图纸标题继续由全新浏览器选择器验证。
- 双品牌 `release:all` 的构建与静态隔离已通过，但全新浏览器一致性检查在音翼基线等待接口接线图时超时。根因是流程先构建音翼、再构建音曼，随后才一次性验证两品牌；此时 `dist` 已被音曼编译结果覆盖，无法作为音翼基线。这是发布编排阻塞问题，不是音翼最终包缺图，也不涉及设备或接线规则。
- 修复边界：每个品牌完成构建、单文件和通用包生成后，立即用同品牌当前 `dist` 依次运行静态验证与浏览器业务输出一致性验证；发布脚本增加双品牌逐一验证契约，避免再次用另一品牌的编译产物作基线。
- 逐品牌验证后音翼通过，音曼稳定出现基线 `AMIC` 与最终包 `EXTMIC` 差异。继续追踪确认：最终包按规定以发布态空白采集表启动，基线 `dist` 却以开发态默认教室启动；验证脚本仅填写部分字段，未覆盖发布态清空的舞台 / 教学区等参数，因此两侧并非同一完整业务输入。
- 修复边界：浏览器基线加载当前 `dist` 时同步注入发布态标记和当前品牌，再与最终 HTML 一样从干净采集表录入固定参数。该改动只校准发布验证夹具，不修改 AJ200 / AJ350 / AJ600 选择、音箱、阵麦、接线或客户页面初始化规则。
- 发布态基线校准后差异仍存在，进一步定位到音曼通用包的 `transformBrandText`：它对整份内联 JavaScript 执行 `yinyi -> yinman` 全局替换，把 `brandId === "yinyi"` 等内部业务判断也改成了音曼条件。音曼因此误走音翼单线阵分支，从正式 AJ350 / AMIC 退成 AJ200 / EXTMIC；该问题可能影响所有品牌条件分支，必须阻断发布并立即修复。
- 修复边界：删除通用包对内部英文品牌 ID 的全局替换，保留显式客户品牌文案转换、品牌资产隔离、最终 HTML 静态黑名单和浏览器可见文本校验；新增源码断言禁止以后再次改写内部 `yinyi` 标识。该修复恢复既有业务规则，不新增或修改处理器选择规则。
- 修复后完整 `release:all` 通过：音翼、音曼均按同品牌 `build -> single-file -> universal -> static verify -> fresh-browser parity` 顺序完成，最终 HTML 与对应 `dist` 的设备清单、点位图、接口接线图和接口占用表一致；音曼单线阵恢复既有 AJ350 / AMIC 链路。
- 双品牌 Android Chrome Pixel 7（412px）和 iOS Safari iPhone 14（390px）发布兼容测试全部通过：标题 / 品牌 / 发布标记正确，采集表初始值为空，无外部资产、乱码、控制台错误或横向溢出。
- 从两个最终单文件 HTML 再次实际导出报告：`output/pdf/音翼AI售前工具-2.0-导出报告校准.pdf`（`1,218,288` bytes）与 `output/pdf/音曼AI售前工具-2.0-导出报告校准.pdf`（`1,252,501` bytes）。两份均为 5 页 A4、载荷版本 `2.0`，第 4 页接线图内嵌图像为 `2480 x 3508`，其他页为 `1240 x 1754`；最终首页渲染确认页脚清楚显示“请联系FAE”。
- 最终发布目录与 zip：`outputs/音翼AI售前工具-2.0-260720-1` / `.zip`，zip `2,300,207` bytes，SHA-256 `D7183E47ACA4D05EAF73BA59F00C85FE149716CA357571FF95B095A60D338AB1`；`outputs/音曼AI售前工具-2.0-260720-1` / `.zip`，zip `2,896,509` bytes，SHA-256 `DC74B9808AD0052FDF6BFD190AC66F42F190F80211A04F54982BF07A0B8C9A00`。
- 两个 zip 均含 HTML、README 和软件大纲 3 个交付文件；最终目录未发现“张灏”“内部测试版”“内部测试报告”，品牌资产 / 文案隔离静态检查无命中。旧历史发布归档按项目规则保留，不推送 GitHub。
- release checkpoint 前对最新快照做独立集合校验，发现快照脚本报告 `172` 项通过，但 Git 的 NUL 文件清单实际有 `272` 个现存文件；缺失项集中在中文路径，包括历史发布归档、本次双品牌 `2.0` 目录 / zip、中文资料和桌面上传脚本。当前 `172` 项快照判定为不完整，不可作为最终回滚点。
- 根因是托管环境只能回退 Windows PowerShell 5.1 时，PowerShell 直接消费 Git UTF-8 原生输出发生编码转换，中文路径损坏后被 `Test-Path` 静默过滤。该问题直接阻塞收工备份，按例外边界立即修复：改为从 Git `-z` 输出的原始字节流按 UTF-8 解码，再拆分 NUL 路径，并在新快照后用独立 Python/zip 集合比对复核。
- 修复后 `scripts/new-daily-snapshot.ps1` 在 Windows PowerShell 5.1 成功生成 `snapshot-20260720-171138-322.zip`，脚本内校验与独立 Git NUL / zip 集合校验均为 `272` 项且零缺失、零多余；不完整的 172 项旧快照已在新快照验证成功后按只保留最新一份规则删除。
## 2026-07-20 软件著作权登记材料整理

- 用户提供代理资料要求：软件名称、软件说明书，以及前 60 页和后 60 页源代码；源码每页 50 行，后册末页需带结束语，材料不得出现申请人以外的人名、单位名或 Logo 等版权标识。
- 本次采用中性登记名称“智能音频售前工程方案设计软件 V2.0”，简称“智能音频售前工具”，没有在材料中写入音翼、音曼、个人姓名、单位名或品牌 Logo。
- 新增 `scripts/generate-software-copyright.py`：
  - 从 57 个 `src` 源码文件建立确定性的匿名化源码序列，共 31,470 个展示行；
  - 排除产品来源目录、在线服务入口、构建产物、第三方依赖、日志、测试输出和图片资产；
  - 中性替换品牌标识、个人姓名、单位名、网址、本机路径和 Logo 标识；
  - 生成前册 3,000 行、后册 2,999 行加 `// 源程序代码结束`，两册均严格 60 页 x 50 行；
  - 生成 4 份 DOCX，并调用文档工具清理作者、最后修改者、custom properties 和 rsid 元数据。
- 从 5174 当前真实功能页采集参数采集、项目档案、设备推荐、点位图、系统拓扑图、接口接线图和接口占用表素材；裁去品牌头，项目字段改为“示例教室项目”，说明书图片不含品牌 Logo。
- 生成材料：
  - `01-软著基本信息与提交清单.docx/pdf`，PDF 4 页；
  - `02-智能音频售前工程方案设计软件V2.0-软件说明书.docx/pdf`，PDF 16 页；
  - `03-源代码文档-前60页.docx/pdf`，60 页，每页 50 行；
  - `04-源代码文档-后60页.docx/pdf`，60 页，每页 50 行，最后一行是结束语；
  - `README-提交前补充信息.txt`；
  - `智能音频售前工程方案设计软件V2.0-软著材料-20260720.zip`。
- 新增 `scripts/render-software-copyright-pdf.py`。本机未找到 LibreOffice、Word 或 WPS，官方 DOCX 渲染器无法启动；按文档技能允许的缺少 `soffice` 边界保留 DOCX 结构校验，同时使用 bundled Python/ReportLab 从 DOCX 内容生成可提交 PDF。
- 新增 `scripts/verify-software-copyright.py`：使用 pypdf 验证 PDF 页数和文本，验证两册每页均提取到 50 个源码行；使用 bundled Poppler 将全部 140 页 PDF 渲染为 PNG，检查页面非空、尺寸一致、边缘无裁切，并生成人工检查联系表；最终 ZIP 已执行完整性校验。
- 所有 DOCX 可见文本和 PDF 提取文本均通过禁用人名、单位名、品牌名扫描；DOCX custom properties 和个人作者元数据为空。
- 申请人法定名称、证件信息、联系信息、开发完成日期和首次发表状态未在用户资料中提供。本次没有虚构主体，已在基本信息清单和 README 中列为提交前必填项。
- 环境记录：托管执行器继续按项目规则使用 `cmd.exe` 兼容回退；`cmd.exe` 输出中文文件名时出现终端显示乱码，但 `rg --files`、Python UTF-8 读取、DOCX/PDF 内容及实际文件名均正常，未重写项目中文文件。

## 2026-07-20 软件著作权长截图与说明书替换

- 用户明确拒绝拼接截图，要求按界面序号和 DOM 实际长度直接截取；并补充要求将 `03 方案输出` 的正文、点位图和系统拓扑图拆成三张独立图片。
- 新增 `scripts/capture-software-copyright-screenshots.mjs`，使用本机 Edge 和 Playwright `locator.screenshot()` 在独立浏览器上下文中录入匿名示例项目，直接截取功能区完整 DOM 容器，不读取或改写用户的 5174 草稿。
- 最终 7 张独立长截图为：售前采集 `1180x1721`、项目档案 `1180x843`、03 方案输出正文 `1180x774`、点位图 `1142x1389`、系统拓扑图 `1142x1529`、接口接线图 `1180x1587`、接口占用表 `1142x736`。接口接线图通过临时隐藏占用表独立截取；03 正文通过临时隐藏点位图和拓扑图独立截取。
- 截图脚本首轮暴露两个阻塞错误并已当场修复：输出路径误用了不存在的 `section.output`；`page.addStyleTag()` 返回的元素句柄不能直接调用 `remove()`，改为在页面上下文执行 `element.remove()`。这两个问题仅影响新截图工具，不涉及售前业务规则。
- `scripts/generate-software-copyright.py` 已改为直接使用上述长截图并重新保存 RGB PNG，不再裁剪旧视口图或覆盖像素；说明书图 3 标题改为“方案输出正文：点位校核、客户选型与设备清单”。图片按宽高上限等比缩放，完整画面不裁切。
- `scripts/render-software-copyright-pdf.py` 的标准文档图片高度上限调整为 `210 mm`，使长图、图注和章节标题能够留在同页；同时移除 9.1 接线图与 9.2 接口占用表之间的旧强制分页，避免两行说明单独占页。
- `scripts/verify-software-copyright.py` 新增说明书内嵌图片校验：必须恰好包含当前 7 张截图，并逐一匹配原始像素尺寸，防止旧截图或拼接图回流。
- 重新生成整套 DOCX、PDF 和 `智能音频售前工程方案设计软件V2.0-软著材料-20260720.zip`。最终说明书 PDF 为 19 页、`1,593,716` bytes；ZIP 为 `3,173,046` bytes。
- 最终校验通过：说明书内嵌 7 张长截图；基本信息 PDF 4 页；前后源码册各 60 页、每页 50 行，共 6000 行；后册末行保留结束语；全部 143 页 PDF 渲染非空、尺寸一致、无边缘裁切；禁用姓名、单位名和品牌标识无命中；ZIP 内容和 CRC 完整。
- 全页视觉复核确认：03 正文位于说明书第 9 页，点位图第 10 页，系统拓扑图第 12 页，三者相互独立；接口接线图和接口占用表分别位于第 14、15 页；所有图注与对应图片同页，无截图截断、拼接缝或叠图。
- Poppler 渲染标准文档时仍输出既有的 `No display font for Symbol / ArialUnicode` 非阻塞提示，进程退出码为 0，143 张页面 PNG、中文正文和截图均正常；未借本次截图替换改动字体配置。
- 本轮首次显式启动 PowerShell 7 仍被托管执行器重定向到无权限 WindowsApps alias 并报 `CreateProcessAsUserW failed: 5`，随后按项目规则改用 `cmd.exe` 与 UTF-8 工具；文件内容正常，未误判为乱码。

## 2026-07-20 发布版本自动同步与产品说明书规则

- 用户确认新的底层发布要求：以后版本号自动更新；正式发布包删除“软件大纲”，改为增加客户可读的“产品说明书”；要求同时写入项目底层规则。
- 当前缺陷已定位：`2.0` 分别写死在单文件打包、通用包、静态验证、浏览器一致性验证、移动兼容验证和 PDF 报告载荷中；只修改其中一处会造成页面标记、目录名、报告和验证脚本版本不一致。
- 当前通用包固定生成并交付“软件大纲.md”，内容含内部 1.x / 3.0 / 4.0 开发路线，不适合作为客户正式交付文档。新发布包应只保留单文件 HTML、打开说明和产品说明书，历史发布归档不追溯改写。
- 实现边界：`package.json` 作为唯一版本源；下一次正式 `release:*` 在当前版本已完成对应品牌发布时自动递增次版本，同一版本发布中断后的重跑复用当前版本，避免失败重试重复跳号；本轮只建设流程，不立即 bump 或生成新发布包。
- 发布流水线回归首次加固“目录 + 主 HTML + zip”完成判定后，测试夹具的旧版本发布标记也被成品目录查找计入，导致候选数由 1 变 2。该失败只存在于临时测试检索条件；正式脚本的版本判定输出仍正确。测试候选随后收紧为必须包含产品说明书的真实新包。
- 已完成统一版本链路：构建常量、PDF 报告载荷、单文件发布、通用包、静态验证、浏览器一致性验证和移动兼容验证均读取 `package.json`；版本写入会同步更新 `package-lock.json`，非法 `major / minor / patch` 参数会直接拒绝。
- 已完成交付文档替换：新发布目录与 zip 严格只允许品牌 HTML、`README-打开说明.txt` 和品牌产品说明书 3 个文件；README 引用说明书，验证脚本检查版本、品牌隔离、必要章节、禁用文案和软件大纲残留。
- 最终验证通过：全部相关 `.mjs` 语法检查、`release:version:dry-run`、`test:release-pipeline`、`test:interface-wiring`、`git diff --check` 和生产构建。dry-run 显示双品牌 `2.0` 已发布，下一次正式发布将从 `2.0.0` 更新为 `2.1.0`；真实 `package.json` 与 `package-lock.json` 仍保持 `2.0.0`，本轮未生成 2.1 发布包。
- 生产构建首次在托管沙箱内因 esbuild 无权读取 `vite.config.ts` 上层路径失败，按既有项目规则在沙箱外重跑后通过；仍只有既有主包约 `709.90 kB` 超过 `500 kB` 的非阻塞体积提示，本轮未顺手拆包。
