...
const singleFileReleaseSource = readFileSync("scripts/build-single-file-release.mjs", "utf8");
const universalReleaseSource = readFileSync("scripts/build-universal-release.mjs", "utf8");
const releaseVersionSource = readFileSync("scripts/release-version.mjs", "utf8");
const bumpReleaseVersionSource = readFileSync("scripts/bump-release-version.mjs", "utf8");
...
assert.match(engineeringAppSource, /请联系FAE/);
assert.doesNotMatch(engineeringAppSource, /张灏/);
...
assert.match(pdfExporterSource, /const interfaceWiringImageScale = 4/);
assert.match(pdfExporterSource, /const interfaceWiringPageScale = 2/);
assert.match(pdfExporterSource, /svgToPngDataUrl\(interfaceWiringSvg, \{ scale: interfaceWiringImageScale \}\)/);
assert.match(pdfExporterSource, /renderImagePage\("接口接线图", interfaceWiringImage, \{[\s\S]*?renderScale: interfaceWiringPageScale,[\s\S]*?jpegQuality: 0\.98/);
...
assert.match(pdfExporterSource, /请联系FAE/);
assert.doesNotMatch(pdfExporterSource, /张灏/);
...
assert.match(imageExporterSource, /options: \{ scale\?: number \} = \{\}/);
...
assert.match(pdfExporterSource, /version: __APP_VERSION__/);
assert.match(packageJsonSource, /bump-release-version\.mjs --brand all/);
assert.match(packageJsonSource, /release:package:yinyi/);
assert.match(packageJsonSource, /release:package:yinman/);
...
assert.match(packageJsonSource, /verify:release-current -- --brand yinyi/);
assert.match(packageJsonSource, /verify:release-behavior -- --brand yinyi/);
assert.match(packageJsonSource, /verify:release-current -- --brand yinman/);
assert.match(packageJsonSource, /verify:release-behavior -- --brand yinman/);
...
assert.doesNotMatch(universalReleaseSource, /replace\(\/yinyi\/g/);
assert.doesNotMatch(universalReleaseSource, /replace\(\/Yinyi\/g/);
assert.match(singleFileReleaseSource, /getReleaseVersion/);
assert.match(universalReleaseSource, /产品说明书/);
assert.match(universalReleaseSource, /expectedReleaseFiles/);
assert.doesNotMatch(universalReleaseSource, /软件开发大纲/);
assert.match(releaseVersionSource, /package\.json/);
assert.match(releaseVersionSource, /package-lock\.json/);
assert.match(bumpReleaseVersionSource, /currentVersionIsPublished/);
assert.match(bumpReleaseVersionSource, /action: shouldBump \? "bump" : "reuse"/);
assert.match(releaseCurrentSource, /productManualMissingRequired/);
assert.match(releaseCurrentSource, /hasNoSoftwareOutlineFile/);
assert.match(releaseCurrentSource, /"接口接线图"/);
assert.match(releaseBehaviorSource, /const wiringSelector = [^\r\n]*brandLabel[^\r\n]*接口接线图/);
assert.match(releaseBehaviorSource, /window\.__YIOU_RELEASE_BUILD__ = true/);
assert.match(releaseCurrentSource, /"张灏"/);
assert.match(releaseBehaviorSource, /customerText\.includes\("张灏"\)/);
...
assert.match(wiringPreviewSource, /function getDirectCableCurve/);
assert.match(wiringPreviewSource, /getTerminalFanMergePoint/);
assert.doesNotMatch(wiringPreviewSource, /routeCableSet|findGridPath|CABLE_ROUTING_CLEARANCE_STEPS/);
...
assert.match(wiringPreviewSource, /data-route-kind=\{drawing\.route\.kind\}/);
assert.match(wiringPreviewSource, /data-route-points=/);
...
assert.match(wiringPreviewSource, /getEdgeInteractionStateClass/);
assert.match(wiringPreviewSource, /marker\.terminalSelective \? " is-terminal-block" : " is-connector"/);
assert.match(wiringPreviewSource, /data-focus-kind=\{marker\.terminalSelective \? "terminal" : "connector"\}/);
assert.match(wiringPreviewSource, /marker\.terminalSelective \? \([\s\S]*?className="interfaceWiringPortImageFocusTint"[\s\S]*?\) : null/);
...
assert.match(wiringPreviewStyles, /@keyframes interfaceWiringCableFocusPulse/);
assert.match(wiringPreviewStyles, /\.interfaceWiringPortImageFocus\.is-active[\s\S]*?opacity: 1;/);
assert.match(wiringPreviewStyles, /interfaceWiringPortImageFocusLayer[\s\S]*?rgba\(255, 255, 255, 0\.96\)/);
assert.match(wiringPreviewStyles, /@keyframes interfaceWiringPortImageFocusPulse[\s\S]*?rgba\(56, 189, 248, 0\.9\)/);
assert.match(wiringPreviewStyles, /\.interfaceWiringPortImageFocus\.is-terminal-block \.interfaceWiringPortImageFocusLayer/);
assert.match(wiringPreviewStyles, /@keyframes interfaceWiringTerminalFocusPulse[\s\S]*?brightness\(1\.02\)[\s\S]*?brightness\(1\.08\)/);
